...
pub fn run(args: Args) -> Result<(), Box<dyn Error>> {
    ...
    let req_thread_exec_state = match &args.awake_mode {
        AwakeMode::Display => {
            println!("Requested ``{}`` mode, meaning the machine will not go to sleep and the display will remain on", Color::Green.paint("Display"));
            ExecutionState::ES_DISPLAY_REQUIRED
        },
        AwakeMode::System => {
            println!("Requested ``{}`` mode, meaning the machine will not go to sleep", Color::Green.paint("System"));
